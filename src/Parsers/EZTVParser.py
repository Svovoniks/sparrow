...
class EZTVParser(ParserBase):
    ...
    def load_page(self, url, cookies={"layout": "def_wlinks"}):
        return super().load_page(url, cookies)
    ...
    def get_all_show_episodes(self, show: Show, limit=200, stop_after=None):
        ...
        pattern_row = r'''<tr name="hover" class="forum_header_border"[\s\S]+?</tr>'''
        pattern_epinfo = r'''<a href="([^"]+)" title="[^"]+" alt="([^"]+)\(([^"]+)\)" class="epinfo">[^"]+</a>'''
        pattern_magnet = r'''"(magnet:[^"]+)"'''
